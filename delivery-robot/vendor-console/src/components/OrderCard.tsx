...
export default function OrderCard({ order, isNew, onUpdateStatus }: OrderCardProps) {
  ...
  return (
    <div
      className={[
        "rounded-lg border bg-zinc-900 p-3 flex flex-col gap-2.5 transition-all duration-200",
        isNew
    ...
    >
      ...
      {/* Items */}
      <ul className="rounded bg-zinc-800/70 border border-zinc-700/50 px-2.5 py-2 flex flex-col gap-1.5">
        {order.items.map((item) => (
          <li key={item.id} className="flex items-baseline gap-1.5 flex-wrap">
            <span className="font-mono text-[11px] font-semibold text-zinc-500 w-5 shrink-0">
              {item.quantity}×
            </span>
            <span className="text-[12px] text-zinc-200">{item.name}</span>
            {item.customisation && (
      ...
      {order.status === "NEW" && (
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 py-1.5 rounded text-[12px] font-semibold text-red-400 bg-zinc-800 border border-red-500/25 hover:bg-red-500/10 transition-colors"
          >
            Reject
          </button>
          <button
      ...
      {nextAction && (
        <button
          ...
          className="w-full py-1.5 rounded text-[12px] font-semibold text-zinc-200 bg-zinc-800 border border-zinc-700 hover:border-blue-500 hover:text-blue-400 transition-colors"
        >
          {nextAction.label} →
        </button>
      )}
      ...
      {order.status === "REJECTED" && (
        <p className="font-mono text-[10px] text-zinc-600 line-through">
          Order rejected
        </p>
      )}
    </div>
